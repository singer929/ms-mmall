...
@Component
@Scope("prototype")
public class MallSpecificationParser extends IGeneralParser{
	 ...
	 * 搜索列表的静态变量名
	 */
	public static final String 	LIST_ARTICLE = "articleList",PRODUCT_SEARCH_LIST_ARTICLE="productSearchList";
	...
	public String parseProduct(){
		...
		htmlContent = new ProductCodeParser(htmlContent,product.getProductLinkUrl()).parse();
		...
		//替换商品的栏目链接标签
		ProductTypeLinkParser plp =new  ProductTypeLinkParser(htmlContent,this.getWebsiteUrl() + column.getColumnPath() + File.separator + IParserRegexConstant.HTML_INDEX);
		if(plp.isTop()){
			if(tmp!=null){
				ptp.setNewCotent(this.getWebsiteUrl() + tmp.getColumnPath() + File.separator + IParserRegexConstant.HTML_INDEX);
			}
		}
	...
	public String parseProductList(){
		// 查找当前模版页面拥有多少个列表标签
				...
				for (int i = 0; i < listNum; i++) {
					// 当前列表标签中属性的集合-------------------
					Map<String, String> property = net.mingsoft.mall.parser.impl.ListParser.listProperty(super.htmlContent, false);
					// 取当前标签下的栏目ID
...
//							columnIds.add(columnId);
					...
					if(columnIds==null || columnIds.length==0){
						htmlContent = new net.mingsoft.mall.parser.impl.ListParser(app,htmlContent, null, this.getWebsiteUrl(), property, false, fieldBiz, contentBiz).parse();
						return htmlContent;
					}
					// 数据库中该栏目下文章的总数
					int productCount =productBiz.getProducntSpecificationCount(columnIds, app.getAppId(),flag,noFlag);
					// 如果没有指定文章每页显示数量则显示所有数量
					if (size <= 0 || size > productCount) {
						size = productCount;
					}
					// 当数据库中该栏目下没有该文章时不取数据
				if (productCount != 0) {
						...
						// 从数据库取出文章列表数组
						List<ProductEntity> productList = productBiz.queryProducntSpecificationList(app.getAppId(), columnIds, 0, size, orderBy, order.equals("desc") ? true : false,flag,noFlag);
						// 替换列表标签
						htmlContent = new net.mingsoft.mall.parser.impl.ListParser(app,htmlContent, productList, this.getWebsiteUrl(), property, false, fieldBiz, contentBiz).parse();
				}else{
					htmlContent = new net.mingsoft.mall.parser.impl.ListParser(app,htmlContent, null, this.getWebsiteUrl(), property, false, fieldBiz, contentBiz).parse();
				}
		}
	...
	public String parseList(){
		...
		if(!StringUtil.isBlank(isPaging)&& isPaging.equals("true")){
			...
			if(productCount!=0){
				page = new PageUtilHtml(curPageNo, size, productCount, listLinkPath);
				List<ProductEntity> productList = productBiz.queryProducntSpecificationList(app.getAppId(), columnIds, (page.getPageNo() * page.getPageSize()), page.getPageSize(), orderBy, order.equals("desc") ? true : false,flag,noFlag);
				htmlContent =new  net.mingsoft.mall.parser.impl.ListParser(app,htmlContent, productList,  this.getWebsiteUrl(), property, true, fieldBiz, contentBiz).parse();
			}else{
				page = new PageUtilHtml(curPageNo, size, productCount, listLinkPath);
				htmlContent =new  net.mingsoft.mall.parser.impl.ListParser(app,htmlContent, null,  this.getWebsiteUrl(), property, true, fieldBiz, contentBiz).parse();
			}
		}
		return htmlContent;
	}
		...
		if (column != null) {
			...
			for (int i = 0; i < strNumType; i++) {
				...
				if (tempColumnId != 0) {
					...
					// 替换栏目标签
					htmlContent = new ChannelParser(htmlContent, categoryList, this.getWebsiteUrl(), column != null ? column.getCategoryId() : 0, mapProperty.get(ChannelParser.CHANNEL_CLASS)).parse();
				} else {
...
//					columnIds.add(column.getCategoryId());
			...
			int size = StringUtil.string2Int(property.get(ListParser.LIST_SIZE));
			// 数据库中该栏目下文章的总数
			int articleCount =  productBiz.getProducntSpecificationCount(columnIds, app.getAppId(),null,null);
...
//				columnIds.add(columnId);
				// 数据库中该栏目下商品的总数
				productCount = productBiz.getProducntSpecificationCount(columnIds, app.getAppId(),null,null);
				listProduct = productBiz.queryProducntSpecificationList(Integer.valueOf(app.getAppId()), columnIds, 0, size, orderBy, order.equals("desc") ? true : false,null,null);
			} 
			// 当数据库中该栏目下没有该文章时不取数据
			if(listProduct!=null){
				/**
				...
				// 替换列表标签
				htmlContent = new net.mingsoft.mall.parser.impl.ListParser(app,htmlContent, listProduct,  this.getWebsiteUrl(), property, true, fieldBiz, contentBiz).parse();
			}
