...
<@ms.content>
	...
	<@ms.contentBody width="85%" style="overflow-y: hidden;">
		<@ms.contentPanel  style="margin:0;padding:0;overflow-y: hidden;">
				...
				<iframe src="${managerPath}/category/list.do?modelId=${modelId?default(0)}&categoryCategoryId=0" style="width:100%;maring:0;padding:0;border:none;height:100%;background-image: url(${skin_manager_loadding});  background-repeat: no-repeat;  background-position: center;display:none" id="listFrame" target="listFrame"></iframe>
...
function getZtreeId(event,treeId,treeNode){
	...
	if(treeNode.check_Child_State==-1){
		$("#listFrame").show();
		$(".alertInfo").hide();
		$("#listFrame").attr("src","${managerPath}/category/"+treeNode.categoryId+"/childList.do?modelId=${modelId?default(0)}&categoryCategoryId="+treeNode.categoryId);
	}else{
		$("#listFrame").hide();
		$(".alertInfo").show();
	}
